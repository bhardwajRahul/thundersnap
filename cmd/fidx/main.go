...
var (
	...
	outfile = getopt.StringLong("outfile", 'o', "", "filename to write fidx data (only works with a single input file)")
...
func main() {
	...
	if *outfile != "" && len(args) > 1 {
		fmt.Fprintln(os.Stderr, "error: --outfile only works with a single input file")
		os.Exit(1)
	}
